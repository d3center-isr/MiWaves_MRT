...
for(var_name in rhs.vars.linear.me) {
  gee_formula <- as.formula(paste0("prop_awakeuse ~ ",var_name," + morning + prop_awakeuse_prior"))
  data <- df %>%
...
for(var_name in rhs.vars.linear) {
  ...
  lm_formula <- paste0("canndays_pt ~ ",var_name) 
  ...
  lm_formula <- paste0("canndays_fu ~ ",var_name) 
  ...
  lm_formula <- paste0("avg_prop_awakeuse ~ ",var_name) 
...
cs_reg_results %>%
    kable(
    align = "c", caption = "Univariate associations of variable with cannabis use days at post-test and at 2-month follow up and with avg. prop. of waking hours using cannabis, using cross-sectional linear regressions")%>%
  kable_classic(full_width = F)
```
