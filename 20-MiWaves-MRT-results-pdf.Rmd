...
res_table <- function(res){
    res %>%
      ...
      kableExtra::footnote(
        general = "Precision covariates include the following: time of day, day of week,", general_title = "", footnote_as_chunk = T
      ) %>%
      kableExtra::footnote(
        general = "Notes: Standard errors are not yet adjusted to account for RL uncertainty.", general_title = "", footnote_as_chunk = T
      ) 
...
- ERQ 1A. Examine whether there is a correlation between the cumulative number of prompts and proximal cannabis use, over time. Refer to *cumul_num_message*.
  
- ERQ 1B. And examine whether there is a correlation between the total number of prompts a person received and cannabis use at the end of the study and at 2-month follow up. 
  
- ERQ 2A. Examine whether there is a correlation between prior engagement and proximal cannabis use, over time. Refer to *prior_interv_engag*,*cov_interv_engag_72hrs*, *cov_interv_engag_48hrs*, *cov_interv_engag_24hrs*.
  
- ERQ 2B. And examine whether a correlation exists between average engagement and cannabis use at the end of the study and at 2-month follow up.
  
- ERQ 3. Examine whether there is a correlation between the mean proportion of cannabis use during waking hours (over study days) and cannabis use days reported at follow up.
...
rhs.vars.linear.me <- c("cumul_num_message","prior_interv_engag","cov_interv_engag_72hrs", "cov_interv_engag_48hrs", "cov_interv_engag_24hrs")

library(lmerTest) # renders p-values
...
for(var_name in rhs.vars.linear.me) {
  ...
  lmm_formula <- paste0("prop_awakeuse ~ ",var_name," + morning + (1 | id)") # random intercept and adjusting for time of day
  res_lmm <- lmer(formula = lmm_formula, data = df)
...
reg_results %>%
    kable(
    align = "c", caption = "Table 4.1. Univariate associations of variable with proximal proportion of waking hours using cannabis, adjusting for time of day and using linear mixed effects regression with a random intercept")%>%
  kable_classic(full_width = F)
