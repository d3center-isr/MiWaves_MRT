...
- ERQ 5. Examine whether there is a quadratic trend in cumulative number of messages in terms of the proximal proportion of waking hours using cannabis outcome, over time. Refer to *cumul_num_message* Table 19.
  
- ERQ 6. Examine interaction term with baseline motivation to change, for the aggregate regressions for post-test, follow-up, and average across study time period. Refer to Table 20.
  
- ERQ 7. Examine interaction between engagement (in forms of engagement score and activity question response) and cannabis use days at baseline, in terms of the three cannabis use aggregate outcomes. Refer to Table 21.
```{r, tidy.opts=list(width.cutoff=60), tidy=TRUE, echo=FALSE}  
...
\newpage
```{r, tidy.opts=list(width.cutoff=60), tidy=TRUE, echo=FALSE} 
# examine interaction between baseline cannabis use and the IV
# Average engagement (IV) on 2 month follow up cannabis use (DV)
# Average behavioral activity (IV) on 2 month follow up cannabis use (DV)

cs_reg_output <- data.frame()
rhs.vars.linear <- c("avg_engagement_multi", "avg_beh_activity_response")
...
cs_reg_results <- cs_reg_output %>%
  ...
  #filter(RHS_variable == Term) %>%
  arrange(Order) %>%
  filter(!((Order==3) & (Term=="avg_prop_awakeuse"))& Term!="(Intercept)") %>%
  select(Outcome,Term,Estimate,SE,UI_lower,UI_upper,pvalue)


cs_reg_results %>%
    kable(
    align = "c", caption = "Baseline cannabis use days interaction with each engagement variable, for cannabis use days at post-test (V1), at 2-month follow up (V2) and with avg. prop. of waking hours using cannabis (V3), using cross-sectional linear regressions")%>%
  kable_classic(full_width = F)
...
First, let us examine the bearing of message interaction type A=1 (acknowledge) / B=2 (acknowledge + click additional resources link) / C=3 (acknowledge + fill in the blank/select an option) on proximal proportion of waking hours using cannabis, as well as the same for message length short=1 (150 characters or less) / long=2 (200 characters or more).  
  
Notes: All continuous covariates are mean-centered. The continuous proximal engagement outcome is also mean-centered for interpretation of the fully marginal causal excursion effect. 
```{r, tidy.opts=list(width.cutoff=60), tidy=TRUE, echo=FALSE}  
...
df_categ <- df %>%
  ...
  dplyr::mutate(A_inter_type = ifelse(actioni==0,0,as.numeric(message_interaction_type)),
         ...
         # create additional version where reference category is coded as "long message" as opposed to "no message" in order to assess difference in effect attributed to short versus long messages
         A_length_type_reflong = ifelse(actioni==0,2,
                                ifelse(message_length == "Short",1,
                                       ifelse(message_length == "Long", 0, 999))),
         prob_A_length_type = ifelse(actioni==0, probi, probi * (1/2)),
         # adjust class to be compatible with source script
         week_day_binary = as.numeric(week_day_binary),
         timeofday = as.integer(timeofday),
         I = 1) 

#categorical treatment (no message=0, interaction type A=1, B=2, C=3)
fit.1  <- wcls_categorical_treatment(
  dta = df_categ,
...
print("beta causal excursion effect estimates for message interaction type,")
print("where 0=no message, 1=A (acknowledge), 2=B (click link), 3=C (fill-in-blank):")
round(fit.1$beta_hat,digits=4)
print("standard errors:")
round(fit.1$beta_se_adjusted,digits=4)
print("95% confidence intervals:")
round(fit.1$conf_int_adjusted_t,digits=4)

#categorical treatment (no message=0, short length message=1, long message=2)
fit.2  <- wcls_categorical_treatment(
  dta = df_categ,
  decision_time_varname = "decision_point",
  id_varname = "id",
  treatment_varname = "A_length_type",
  outcome_varname = "prop_awakeuse_c",
  control_varname = c("cov_prop_awakeuse_48hrs_c", "cov_interv_engag_24hrs_c", "timeofday", "week_day_binary", "cann_importance_bl_c", "cannhours_bl_c", "cannwake_bl_c"),
  moderator_varname = c(),
  rand_prob_varname = "prob_A_length_type",
  estimator_initial_value = NULL,
  trt_level = 2
)
print("beta causal excursion effect estimates for message length type,")
print("where 0=no message, 1=short (<=150 characters),  2=long (>=200 chararacters):")
round(fit.2$beta_hat,digits=4)
print("standard errors")
round(fit.2$beta_se_adjusted,digits=4)
print("95% confidence intervals")
round(fit.2$conf_int_adjusted_t,digits=4)

```

Next, let us again examine the bearing of message interaction type A=1 (acknowledge) / B=2 (acknowledge + click additional resources link) / C=3 (acknowledge + fill in the blank/select an option) except for on proximal intervention engagement. Then, similarly, we examine the same for message length short=1 (150 characters or less) / long=2 (200 characters or more). The results below reflect an analogous sample size as for proximal cannabis use, that is the additional restriction is applied of removing decision points without an EMA completion.
...
#categorical treatment (no message=0, interaction type A=1, B=2, C=3)
fit.3  <- wcls_categorical_treatment(
  dta = df_categ,
  decision_time_varname = "decision_point",
  id_varname = "id",
  treatment_varname = "A_inter_type",
  outcome_varname = "engagement_multi_c",
  control_varname = c("timeofday", "week_day_binary", "cov_interv_engag_72hrs_c", "cov_humtch_binary_48hrs_c"),
  moderator_varname = c(),
  rand_prob_varname = "prob_A_inter_type",
  estimator_initial_value = NULL,
  trt_level = 3
...
print("beta causal excursion effect estimates for message interaction type,")
print("where 0=no message, 1=A (acknowledge), 2=B (click link), 3=C (fill-in-blank):")
round(fit.3$beta_hat,digits=4)
print("standard errors:")
round(fit.3$beta_se_adjusted,digits=4)
print("95% confidence intervals:")
round(fit.3$conf_int_adjusted_t,digits=4)

#categorical treatment (no message=0, short length message=1, long message=2)
fit.4  <- wcls_categorical_treatment(
  dta = df_categ,
  decision_time_varname = "decision_point",
  id_varname = "id",
  treatment_varname = "A_length_type",
...
print("beta causal excursion effect estimates for message length type,")
print("where 0=no message, 1=short (<=150 characters),  2=long (>=200 chararacters):")
round(fit.4$beta_hat,digits=4)
print("standard errors")
round(fit.4$beta_se_adjusted,digits=4)
print("95% confidence intervals")
round(fit.4$conf_int_adjusted_t,digits=4)


#categorical treatment (long length message=0, short message=1, no message=2), where change reference category
fit.4  <- wcls_categorical_treatment(
  dta = df_categ,
  decision_time_varname = "decision_point",
  id_varname = "id",
  treatment_varname = "A_length_type_reflong",
  outcome_varname = "engagement_multi_c",
  control_varname = c("timeofday", "week_day_binary", "cov_interv_engag_72hrs_c", "cov_humtch_binary_48hrs_c"),
  moderator_varname = c(),
  rand_prob_varname = "prob_A_length_type",
...
print("beta causal excursion effect estimates for message length type,")
print("where 0=long (>=200 chararacters), 1=short (<=150 characters),  2=no message:")
round(fit.4$beta_hat,digits=4)
print("standard errors")
round(fit.4$beta_se_adjusted,digits=4)
print("95% confidence intervals")
round(fit.4$conf_int_adjusted_t,digits=4)
```
The results below reflect the engagement sample size, that does not additionally remove decision points without an EMA completion.
  
Notes: All continuous covariates are mean-centered. The continuous proximal engagement outcome is also mean-centered for interpretation of the fully marginal causal excursion effect. 
```{r, tidy.opts=list(width.cutoff=60), tidy=TRUE, echo=FALSE}  

source("wcls_categorical_treatment.R")
## examine bearing of message interaction type A, B, C on proximal engagement, as well as the same for message length long, short
# create additional categorical treatement variable and adjust probabilities
df_engag_categ <- df_engag %>%
  # remove additional 9 observations where the message interaction type 
  # and message length are missing from the dashboard
  dplyr::filter(!(is.na(message_id) & actioni==1))%>% # 7,038 -> 7,029
  dplyr::mutate(A_inter_type = ifelse(actioni==0,0,as.numeric(message_interaction_type)),
         prob_A_inter_type = ifelse(actioni==0, probi, probi * (1/3)),
         A_length_type = ifelse(actioni==0,0,
                                ifelse(message_length == "Short",1,
...
#categorical treatment (no message=0, interaction type A=1, B=2, C=3)
fit.5  <- wcls_categorical_treatment(
  dta = df_engag_categ,
  decision_time_varname = "decision_point",
  id_varname = "id",
  treatment_varname = "A_inter_type",
...
print("beta causal excursion effect estimates for message interaction type,")
print("where 0=no message, 1=A (acknowledge), 2=B (click link), 3=C (fill-in-blank):")
round(fit.5$beta_hat,digits=4)
print("standard errors:")
round(fit.5$beta_se_adjusted,digits=4)
print("95% confidence intervals:")
round(fit.5$conf_int_adjusted_t,digits=4)

#categorical treatment (no message=0, short length message=1, long message=2)
fit.6  <- wcls_categorical_treatment(
  dta = df_engag_categ,
  decision_time_varname = "decision_point",
  id_varname = "id",
  treatment_varname = "A_length_type",
...
print("beta causal excursion effect estimates for message length type,")
print("where 0=no message, 1=short (<=150 characters),  2=long (>=200 chararacters):")
round(fit.6$beta_hat,digits=4)
print("standard errors")
round(fit.6$beta_se_adjusted,digits=4)
print("95% confidence intervals")
round(fit.6$conf_int_adjusted_t,digits=4)
